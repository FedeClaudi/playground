...
for region in ['SCm']:
    ...
    # Bin spike counts in 1s bins for persistent homology

    # embedd data into lower dimensional embeddiing spaces
...
# # bin data
# binned = cells.reshape(-1, len(reg_clusters), 250)
# binned = np.sum(binned, axis=-1)

# plt.plot(binned[:, 0])

# # save
# np.save(f'{region}_cells.npy', cells)
# np.save(f'{region}_cells_binned.npy', binned)
# reg.to_hdf(f'{region}_spikes.h5', key='hdf5')
